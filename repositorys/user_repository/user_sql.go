...
func (Ui *UserImplementation) LoginUser(input dto.UserLogin) (dto.UserResponse, error) {
	...
	err = helpers.ComparePassword(input.Password, UserLogin.Password)
...
func (Ui *UserImplementation) FindByEmail(email string) (*model.User, error) {
	user := model.User{}

	result := Ui.db.Where("email = ?", email).First(&user)
	if result.Error != nil {
