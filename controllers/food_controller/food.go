...
func (Fc *FoodController) UpdateFood(c echo.Context) error {
	...
	fileExtension := strings.ToLower(filepath.Ext(uniqueFilename))
	...
	default:
		contentType = "application/octet-stream" // Default Content-Type jika tidak dikenali
	}
...
func (Fc *FoodController) GetFoodByAdminID(c echo.Context) error {
	var food []dto.FoodResponse
	...
	food, err = Fc.FoodService.GetFoodByAdminID(admin_id)
	...
	// Return response if success
	return c.JSON(http.StatusOK, echo.Map{
		"message": "success get food by admin id",
		"data":    food,
	})
